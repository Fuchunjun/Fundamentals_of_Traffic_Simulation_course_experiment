...
for i in range(1, steps+1):
    ...
    if i > int(reaction_time / dt):  # 考虑反应时间
        i = i - 1
        distance = front_position[i] - rear_position[i]
        speed_diff = front_speed[i] - rear_speed[i]
        
        # GHR模型计算后车加速度
        rear_acceleration.append(c * (rear_speed[i]**m) * (speed_diff) / (distance**l))
    else:
    ...
    if rear_position[i] > front_position[i]:
        ...
        break  # 碰撞发生，停止仿真
    print(f"时间 {i * dt:.2f} 秒，后车位置 {rear_position[i]:.2f}, 前车位置 {front_position[i]:.2f}, 后车速度 {rear_speed[i]:.2f}, 前车速度 {front_speed[i]:.2f},前车加速度{front_acceleration:.2f},后车加速度 {rear_acceleration[i]:.2f}")
